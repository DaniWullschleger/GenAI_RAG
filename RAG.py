...
def list_available_embedders(topic):
    ...
    embeddings_files = [f for f in os.listdir(embeddings_dir) if re.match(r'embeddings_.*\.npy', f)]
    # Extract embedder names from filenames
    embedder_names = [re.sub(r'embeddings_(.*)\.npy', r'\1', f) for f in embeddings_files]
    return embedder_names


# Function to load embeddings and documents for a given topic
def load_embeddings_for_topic(topic, embedder_name):
    script_dir = os.path.dirname(os.path.abspath(__file__))
    embeddings_dir = os.path.join(script_dir, 'embeddings', topic)
    embeddings_path = os.path.join(embeddings_dir, f'embeddings_{embedder_name}.npy')
...
def generate_response_with_openai_chat(model, query, embeddings, docs, top_k, temperature, system_content):
...
def main():
    ...
    embeddings, docs = load_embeddings_for_topic(topic, selected_embedder)
    ...
    with st.form("my_form"):
        ...
        if submitted and user_query:
            response = generate_response_with_openai_chat(model, user_query, embeddings, docs, top_k, temperature, user_defined_context)
